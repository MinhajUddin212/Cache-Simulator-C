...
void accessData (
    ...
    /* ... */
    mem_addr_t setIdx = (addr>>b) & 3;
    mem_addr_t tag = addr>>6;
    // Find the correct cache set
    /* ... */
    size_t least_used_index =0;
    size_t temp =program_counter;
    for(size_t line_index = 0; line_index<E; line_index++){
        // Cache hit
        /* ... */
        if((cache[setIdx][line_index].valid == true) && (cache[setIdx][line_index].tag == tag)){
            (*hit_count)++;
            cache[setIdx][line_index].recent_use = program_counter;
            return;
        }else{
            // find LRU
            if(cache[setIdx][line_index].recent_use < temp){
                temp = cache[setIdx][line_index].recent_use;
                least_used_index = line_index;
            }
        }
    }
    ...
    if(cache[setIdx][least_used_index].valid == true){
        // if line is already occupied update contens and evict
        cache[setIdx][least_used_index].tag = tag;
        cache[setIdx][least_used_index].recent_use = program_counter;
        (*eviction_count)++;
    }else{
        // if the line is empty
        cache[setIdx][least_used_index].tag = tag;
        cache[setIdx][least_used_index].recent_use = program_counter;
        cache[setIdx][least_used_index].valid = true;
    }
